<template>
  <div class="bg-gray-100 bg-opacity-100 shadow-lg font-bold z-10">
    <h2 class="title">
      ...
      <a
        class="opacity-0 buy-me-a-coffee"
    ...
    <form class="form mb-10" autocomplete="on">
      ...
      <!-- LANGUAGE-->
      <fieldset class="hidden form__section px-6 py-3">
      ...
      <div class="form__section flex flex-col p-6 gap-3">
        ...
        <button
          type="button"
          class="form__btn flex flex-col justify-center outline-0"
          @click="downloadPdf"
        >
          <span>{{ $t('download-cv-pdf') }}</span>
          <span class="hidden">({{ $t('chrome-recommended') }})</span>
        </button>
      </div>
      <!-- CTA -->
    </form>
